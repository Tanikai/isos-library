...
public class DepVerifyMap {
   ...
   */
  private Map<ReplicaId, DepVerifyMessage> depVerifys;
   ...
   *
   * @param depVerifys List of DepVerify messages from the follower quorum defined in the initial
   *     DepPropose message.
   * @return The hash of the DepVerify message vector.
   */
  public static String calculateDepVerifyHash(Collection<DepVerifyMessage> depVerifys) {
    try {
      MessageDigest digest = MessageDigest.getInstance("SHA-256");
      List<DepVerifyMessage> sorted = new ArrayList<>(depVerifys);
  ...
  public Map<ReplicaId, DepVerifyMessage> getDepVerifys() {
    return Collections.unmodifiableMap(depVerifys);
  }
  ...
  public void setDepVerify(ReplicaId replicaId, DepVerifyMessage depVerify)
    ...
    this.depVerifys.put(replicaId, depVerify);
  ...
  public String getdepVerifyHashCached() {
    if (isHashDirty) {
      this.hash = calculateDepVerifyHash(this.depVerifys.values());
      this.isHashDirty = false;
    }
  ...
  public void clearDepVerifys() {
    this.depVerifys.clear();
  }

  public boolean reachedQuorum(int quorumSize) {
    return this.depVerifys.size() >= quorumSize;
  }
  ...
  public boolean isFpVerified(int maxFaults) {
    // Before we are fp-verified, we need the depVerifys from the 2f followers.
    ...
    return DepVerifyMap.minimumDependencyOccurrence(this.depVerifys.values(), maxFaults + 1);
  }
   ...
   *     If depVerifys is empty, true is always returned.
   */
  public static boolean minimumDependencyOccurrence(
      Collection<DepVerifyMessage> depVerifys, int minCount) {
