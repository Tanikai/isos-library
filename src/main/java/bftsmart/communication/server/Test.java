  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < warmup; i++) {
      ...
      if (id == 0) {
        ...
        scl.send(
            targets,
            new TOMMessage(id, 0, 0, i, msg.getBytes(), 0, TOMMessageType.ORDERED_REQUEST),
            true);
        int rec = 0;
        ...
        scl.send(
            new int[] {m.getSender()},
            new TOMMessage(id, 0, 0, i, m.getContent(), 0, TOMMessageType.ORDERED_REQUEST),
            true);
      }
    }
    ...
    System.out.println(
        "Average time for " + test + " executions (-10%) = " + st.getAverage(true) / 1000 + " us ");
    System.out.println(
        "Standard desviation for "
            + test
            + " executions (-10%) = "
            + st.getDP(true) / 1000
            + " us ");
    System.out.println(
        "Maximum time for " + test + " executions (-10%) = " + st.getMax(true) / 1000 + " us ");
    System.out.println(
        "Average time for "
            + test
            + " executions (all samples) = "
            + st.getAverage(false) / 1000
            + " us ");
    System.out.println(
        "Standard desviation for "
            + test
