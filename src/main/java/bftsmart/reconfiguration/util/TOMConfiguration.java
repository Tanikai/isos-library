...
public class TOMConfiguration extends Configuration {
  ...
  protected void init() {
    ...
    try {
       ...
       * Tulio Ribeiro
       *
       * <p>SSL/TLS configuration parameters. Default values: # keyStoreFile =
       * "EC_KeyPair_256.pkcs12"; # enabledCiphers = new String[] {"TLS_RSA_WITH_NULL_SHA256",
       * "TLS_ECDHE_ECDSA_WITH_NULL_SHA"}; # ssltlsProtocolVersion = "TLSv1.2";
       */
  ...
  /**
   * The maximum size a batch of messages can have in bytes. This limit is useful for performance
   * and memory limiting reasons when handling large requests.
   */
  ...
  /**
   * Indicates if a simple control flow mechanism should be used to avoid an overflow of client
   * requests
   */
  ...
  /**
   * Maximum size in bytes a request from a client may have. Larger messages are discarded. This
   * setting is useful when malicious clients are present.
   */
