...
public class StateLog {
  ...
  private TreeMap<Integer, TOMMessage>
      lastReplies; // For each client id (Integer) remember the last ordered
  ...
  /**
   * Sets the state associated with the last checkpoint, and updates the consensus ID associated
   * with it
   *
  ...
  public void newCheckpoint(byte[] state, byte[] stateHash, int lastConsensusId) {
    ...
    if (messageBatches != null) {
      for (int i = 0; i < this.messageBatches.length; i++) messageBatches[i] = null;
    }
  ...
  /**
   * Adds a message batch to the log. This batches should be added to the log in the same order in
   * which they are delivered to the application. Only the 'k' batches received after the last
   * checkpoint are supposed to be kept
   *
  ...
  public DefaultApplicationState getApplicationState(int cid, boolean setState) {
    ...
    if (cid >= lastCheckpointCID && cid <= this.lastCID) {
      ...
      if (size > 0) {
        ...
        for (int i = 0; i < size; i++) batches[i] = messageBatches[i];
      }
      lastCID = cid;
   ...
   *
   * @param transState TransferableState object containing the information which is used to updated
   *     this log
   */
