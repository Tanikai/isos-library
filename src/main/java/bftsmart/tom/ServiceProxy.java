...
public class ServiceProxy extends TOMSender {
   ...
   * @param replyComparator Used for comparing replies from different servers to extract one
   *     returned by f+1
  ...
  public ServiceProxy(
    ...
    comparator =
        (replyComparator != null) ? replyComparator : (o1, o2) -> Arrays.equals(o1, o2) ? 0 : -1;
    extractor =
        (replyExtractor != null)
            ? replyExtractor
  ...
  /**
   * Get the amount of time (in seconds) that this proxy will wait for servers replies before
   * returning null.
   *
  ...
  /**
   * Get the amount of time (in seconds) that this proxy will wait for servers unordered hashed
  ...
  /**
   * Set the amount of time (in seconds) that this proxy will wait for servers replies before
   * returning null.
   *
  ...
  /**
   * Set the amount of time (in seconds) that this proxy will wait for servers unordered hashed
   * replies before returning null.
   *
  ...
  /**
   * This method sends an ordered request to the replicas, and returns the related reply. If the
   * servers take more than invokeTimeout seconds the method returns null. This method is
   * thread-safe.
   *
  ...
  /**
   * This method sends an unordered request to the replicas, and returns the related reply. If the
   * servers take more than invokeTimeout seconds the method returns null. This method is
   * thread-safe.
   *
   * @param request to be sent
  ...
  /**
   * This method sends an unordered request to the replicas, and returns the related reply. This
   * method chooses randomly one replica to send the complete response, while the others only send a
   * hash of that response. If the servers take more than invokeTimeout seconds the method returns
   ...
   * @param reqType
   *     ORDERED_REQUEST/ORDERED_HASHED_REQUEST/UNORDERED_REQUEST/UNORDERED_HASHED_REQUEST for
  ...
  public byte[] invoke(byte[] request, TOMMessageType reqType) {
    try {
      ...
      if (requestHandler.isRequestTimeout()) {
        logger.info(
            "###### TIMEOUT ({}s) OF REQUEST {} | seqId: {} | replies received: {} ######",
            invokeTimeout,
            reqType,
            requestHandler.getSequenceId(),
            requestHandler.getNumberReceivedReplies());
        if (reqType == TOMMessageType.UNORDERED_HASHED_REQUEST
            || reqType == TOMMessageType.UNORDERED_REQUEST) {
          return invoke(request, TOMMessageType.ORDERED_REQUEST);
        } else {
          return null;
        }
  ...
  private AbstractRequestHandler createRequestHandler(TOMMessageType requestType) {
    ...
    if (requestType == TOMMessageType.UNORDERED_HASHED_REQUEST
        || requestType == TOMMessageType.ORDERED_HASHED_REQUEST) {
      int replyServer = getRandomlyServerId();
      logger.debug(
          "[Client {}] replyServerId({}) pos({})",
          getProcessId(),
          replyServer,
