 ...
 *
 * <p>The methods does not try to enforce any constraint, so be correct when using it.
 */
public final class BatchBuilder {
  ...
  private byte[] createBatch(
    ...
    int size =
        20
            + // timestamp 8, nonces 4, nummessages 4
            (numberOfNonces > 0 ? 8 : 0)
            + // seed if needed
            (Integer.BYTES * numberOfMessages)
            + // messages length
            sigsSize
            + // signatures size
  ...
  private void putMessage(
      ByteBuffer proposalBuffer, byte[] message, boolean addSig, byte[] signature) {
  ...
  public byte[] makeBatch(
      List<TOMMessage> msgs, int numNounces, long timestamp, boolean useSignatures) {
  ...
  public byte[] makeBatch(
      List<TOMMessage> msgs, int numNounces, long seed, long timestamp, boolean useSignatures) {
