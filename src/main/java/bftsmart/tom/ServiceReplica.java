...
/**
 * This class receives messages from DeliveryThread and manages the execution from the application
 * and reply to the clients. For applications where the ordered messages are executed one by one,
 * ServiceReplica receives the batch decided in a consensus, deliver one by one and reply with the
 * batch of replies. In cases where the application executes the messages in batches, the batch of
 * messages is delivered to the application and ServiceReplica doesn't need to organize the replies
 * in batches.
 */
public class ServiceReplica {
  ...
  private ServerCommunicationSystem cs = null; // initialized in init()
  ...
  public ServiceReplica(
      int id, Executable executor, Recoverable recoverer, RequestVerifier verifier) {
  ...
  /**
   * Stops the service execution at a replica. It will shutdown all threads, stop the requests'
   * timer, and drop all enqueued requests, thus letting the ServiceReplica object be
   * garbage-collected. From the perspective of the rest of the system, this is equivalent to a
   * simple crash fault.
   */
  public void kill() {
  ...
  public void receiveMessages(
    ...
    for (TOMMessage[] requestsFromConsensus : requests) {
      ...
      for (TOMMessage request : requestsFromConsensus) {
        ...
        logger.debug(
            "Processing TOMMessage from client "
                + request.getSender()
              ...
              case ORDERED_HASHED_REQUEST:
                ...
                MessageContext msgCtx =
                    new MessageContext(
                        ...
                        request.getReplyServer(),
                        request.serializedMessageSignature,
                        firstRequest.timestamp,
                        request.numOfNonces,
                        request.seed,
                        regencies[consensusCount],
                        leaders[consensusCount],
                        consId[consensusCount],
                        cDecs[consensusCount].getConsMessages(),
                        firstRequest,
                        false);
                ...
                if (executor instanceof BatchExecutable) {
                  ...
                  // above.
                  boolean isReplyHash =
                      request.getReqType() == TOMMessageType.ORDERED_HASHED_REQUEST
      ...
      if (noop && this.recoverer != null) {
        ...
        // MessageContext msgCtx = new MessageContext(-1, -1, null, -1, -1, -1, -1, null, // Since
        // it is a noop, there is no need to pass info about the client...
        //        -1, 0, 0, regencies[consensusCount], leaders[consensusCount],
        // consId[consensusCount], cDecs[consensusCount].getConsMessages(), //... but there is still
        // need to pass info about the consensus
        //        null, true); // there is no command that is the first of the batch, since it is a
        // noop
        // msgCtx.setLastInBatch();
    ...
    if (executor instanceof BatchExecutable && numRequests > 0) {
      ...
      for (TOMMessage m : toBatch) {
        batch[line] = m.getContent();
        isReplyHashes[line] =
            m.getReqType() == TOMMessageType.ORDERED_HASHED_REQUEST
      ...
      TOMMessage[] replies =
          ((BatchExecutable) executor)
              .executeBatch(id, SVController.getCurrentViewId(), isReplyHashes, batch, msgContexts);
  ...
  private void initTOMLayer() {
    ...
    tomLayer =
        new TOMLayer(executionManager, this, recoverer, acceptor, cs, SVController, verifier);
...
//    SVController.setTomLayer(tomLayer);
  ...
  /**
   * Obtains the current replica context (getting access to several information and capabilities of
